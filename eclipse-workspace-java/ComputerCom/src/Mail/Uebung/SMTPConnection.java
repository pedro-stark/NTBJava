...
public class SMTPConnection {
	...
	public SMTPConnection (Envelope envelope) throws IOException {
	  ...
	  sendCommand ("HELO " + localhost, 250);     // TODO
	  isConnected = true;
	...
	private void sendCommand (String command, int expectedRC) throws IOException {
		 ...
		 */
		int realRC = parseReply (reply);
		if (realRC != expectedRC) throw new IOException ();
	} // sendCommand
	...
	private int parseReply (String reply) {
		...
		int replyCode = Integer.parseInt (temp);
		return replyCode;
	} // parseReply
