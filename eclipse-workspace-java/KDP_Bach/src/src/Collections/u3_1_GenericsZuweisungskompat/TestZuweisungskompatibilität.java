...
public class TestZuweisungskompatibilität {
	...
	public static void main(String[] args) {
		...
		Collection<Fahrzeug> arraylistOfFahrzeug = ???; 
		arraylistOfFahrzeug.add(???);
		
		//Kann ein HashSet zugwiesen werden?
		Collection<Fahrzeug> setOfFahrzeug = ???; 
		setOfFahrzeug.add(???);
		
		
		//gegeben ist eine Collection von PKWs. Welche Objekttypen können nun hinzugefügt werden?
		Collection<PKW> alP = new ArrayList<>(); 
		alP.add(???);
		fuhrpark.addAll(alP);
		...
		//
		//1. Rufen Sie für jedes Element die Methode doSomething auf
