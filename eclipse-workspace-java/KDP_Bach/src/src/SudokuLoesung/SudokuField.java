...
public class SudokuField extends JPanel implements ActionListener {
	...
	private int getValue() {
		switch(type) {
		case SudokuField.TYPE_FIELD: return Sudoku.getField(row, col); 
		case SudokuField.TYPE_ROW: return Sudoku.getRow(row); 
		case SudokuField.TYPE_COL: return Sudoku.getCol(col); 
		case SudokuField.TYPE_SQUARE: return Sudoku.getSquare(row, col); 
		}
		return 0;
	}
	...
	public void update() {
		...
		for (int i = 0; i < 9; i++) {
			...
			b.setBackground(b.isEnabled() ? (type == TYPE_FIELD ? Sudoku.getEnteredByUser(row, col) ? Color.orange: Color.GREEN : Color.WHITE) : Color.LIGHT_GRAY);
			v = v >> 1;
		}
	}
	...
	public void actionPerformed(ActionEvent e) {
		...
		Sudoku.setEnteredByUser(row, col, true);
		Sudoku.setField(row, col, 1 << (v-1));
		Sudoku.update();
	}
}
