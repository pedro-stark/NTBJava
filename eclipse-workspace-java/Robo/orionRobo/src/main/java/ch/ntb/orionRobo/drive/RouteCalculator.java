...
public class RouteCalculator {// extends JFrame {
	...
	public static void main(String[] args) throws InterruptedException {
		...
		Point initialPoint = new Point(660 / 5, 105 / 5);
		Point finalPoint = new Point(660 / 5, 205 / 5);
		AStar astar = new AStar(initialPoint, finalPoint, field.field);
		List<Point> list = astar.findPath();

		System.out.println("Size " + list.size());
		for (Point item : list) {

			System.out.println("Row " + item.getRow());
			System.out.println("Row " + item.getCol());

		}

		field = new Field();
		System.out.println("Feldlänge " + field.field.length);
	

		Point initialPoint1 = new Point(660 / 5, 105 / 5);
		Point finalPoint1 = new Point(660 / 5, 205 / 5);
		astar = new AStar(initialPoint1, finalPoint1, field.getField());
		list = astar.findPath();
		System.out.println("Size " + list.size());
...
//		}
	...
	public List<Point> nextPosiblePostionTestMethode(Point[][] points, int nextColValue, Point[][] field) { // ,
																											// Parameter
																											// zum
																											// testen
																											// für
		...
		for (int i = points.length - 1; i >= 0; i--) {
			...
			if (localpath.size() > minRoutePointsValue) {
				...
				for (Point Point : localpath) {
					field[Point.getRow()][Point.getCol()].setPointType(PointType.DRIVING_POINT);
